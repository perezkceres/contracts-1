...
// BetaToken.
contract BetaToken is BEP20('PolyAlpha Finance', 'BETA') {
...
contract BetaMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BETA
    ...
    struct PoolInfo {
        ...
        uint256 accBetaPerShare;   // Accumulated BETAs per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The BETA TOKEN!
    BetaToken public immutable beta;
    // Dev address.
    ...
    event BetaMintError(bytes reason);
    ...
    function pendingBeta(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accBetaPerShare = accBetaPerShare.add(betaReward.mul(1e18).div(pool.lpSupply));
        }
        return user.amount.mul(accBetaPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        try beta.mint(devaddr, betaReward.div(10)) {
        } catch (bytes memory reason) {
            betaReward = 0;
            emit BetaMintError(reason);
        }
        
        try beta.mint(address(this), betaReward) {
        } catch (bytes memory reason) {
            betaReward = 0;
            emit BetaMintError(reason);
        }
        
        pool.accBetaPerShare = pool.accBetaPerShare.add(betaReward.mul(1e18).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accBetaPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accBetaPerShare).div(1e18);
    ...
    function updateEmissionRate(uint256 _BetaPerBlock) external onlyOwner {
        require(_BetaPerBlock <= MAX_EMISSION_RATE, "Too high");
        massUpdatePools();
        BetaPerBlock = _BetaPerBlock;
